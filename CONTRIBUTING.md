...
| `npm run check:module` | One `module.yaml` per module code, correctly homed, with its required payload — and every plugin in `marketplace.json` resolving to an authored `PluginResolver` strategy rather than BMad's silent synthesized fallback |
