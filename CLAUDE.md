...
Every dependency site resolves a **preferred** BMad skill name first and falls back to the
legacy name when the preferred one is absent — no fixed minimum BMad version is required, and
...
`bmad-review-adversarial-general`), `bmad-sprint-planning` (readiness gate, invoked with
`intent=readiness`; legacy fallback `bmad-check-implementation-readiness`).
...
Optional: `bmad-ux` (legacy fallback `bmad-ux-review`) — UX review phases skip gracefully
when neither is present. (`bmad-testarch-atdd` was previously listed here, but no step file
ever invoked it; its gating machinery has been removed.)
