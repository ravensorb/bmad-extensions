...
- `state/.notices.yaml` — the one-per-session advisory ledger `pm-status.py notice --state-root S --session-id SESS --key KEY` reads and writes under `notices_lock`, pruned to the 20 most recent session ids. Backs the `l3io-pm-execute`/`l3io-pm-plan` setup-pointer (`config-resolution.md` §5): exit 0 means "not shown this session yet — show it and record it," exit 1 means "already shown." Never a trigger for setup itself. Kept out of git the same way lock files are — `_ensure_lock_ignore` writes both `*.lock` and `.notices.yaml` into `{state_root}/.gitignore`, one mechanism covering both.
