...
| `list-epics` | `--state-root [--format {keys,json}]` — read-only enumeration of every epic in the state tree with its bucket. `keys` (default) prints one `E{nnn}` per line, sorted, buckets flattened; `json` prints `[{key, bucket, status}, …]` with bucket in `active`/`planned`/`archived` order. An absent or empty state root exits 0 with empty output. |
| `list-stories` | `--state-root --epic E [--sprint S] [--format {keys,json}]` — read-only enumeration of story node files under an epic, optionally scoped to one sprint. Filters out `epic.yaml`/`sprint.yaml` and any `.yaml` whose stem does not parse as `E{nnn}-S{nn}-{nnn}`. Absent epic exits 3; absent sprint (a sprint the epic has not created) exits 0 with empty output. |
...
| `set-depends-on` | `--state-root` and either `--epic E` or `--story KEY`, plus repeatable `--add KEY`. The one LIST-shaped field: epic keys on an epic node, story keys on a story node (`status-files.md` §11) — the wrong shape for the node is refused. `set-field` cannot write it, because a list routed through `--value` is stored as the string `"['E001']"` and a later reader takes it for a scalar. Idempotent and order-preserving; all-or-nothing, since every key is validated before anything is written — a half-applied dependency list silently reorders a plan phase. A sprint node has no `depends_on`: exit 2. |
| `repair-issue` | `--state-root --key K --action {unschedule,link,reseed,reopen,normalize-status}` + `--story` for link, `--all-legacy` for normalize-status (which then takes no `--key`) — structural repairs, each refused (exit 2) unless its `audit-issues` finding holds (1b/1g, 1d, 1e, 1j); `unschedule`/`link` also refuse a key that already has a resolved entry (audit finding 1a: rerun `resolve-issue` to clear the stale open copy); `reseed` merges non-canonical aliases of the epic's `next` key into the canonical key by max and never lowers it, but refuses outright (exit 2, nothing written) when any of the epic's alias or canonical values exceeds 1000 — that case is report-only, fixed by hand; `unschedule` writes an `issue_unscheduled` event; `link` writes an `issue_scheduled` event carrying `via: repair-link` (promote's own `issue_scheduled` event carries no `via`); `normalize-status` maps a pre-3.0 legacy open status (`deferred`) to its current equivalent (`backlog`) and refuses (exit 2) any status not in `LEGACY_OPEN_STATUSES`, writing an `issue_status_normalized` event — `--all-legacy` applies it to every legacy item in one pass under one lock, gating each item individually. |
