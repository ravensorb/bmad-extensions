...
| `set-status`, `set-actual` extras | `--no-events` skips the verb's own event (`set-status`'s `status` event, `set-actual`'s `actual` event); the done hook's `issue_resolved` events are still written; `--session-id ID` stamps it; `set-status --status done` on a story resolves the backlog items its `resolves:` lists (a failure warns, never fails the call) |
