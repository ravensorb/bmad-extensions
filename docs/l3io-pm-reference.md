...
| `set-status`, `set-actual` extras | `--no-events` skips the status event; the done hook's `issue_resolved` events are still written; `--session-id ID` stamps it; `set-status --status done` on a story resolves the backlog items its `resolves:` lists (a failure warns, never fails the call) |
| `set-field` extras | Refuses any field in `DERIVED_NODE_FIELDS`, or any sub-path of one (e.g. `status.x`, `resolves.x`), outright (exit 2) — currently `completion_evidence.tests_passing` (use `add-test-run`), `status` (use `set-status`), and `resolves` (use `promote-issue`), rather than asserted directly |
