...
| `repair-issue` | `triage` | `unschedule`, `reopen`, `link`, `reseed` and `normalize-status` repairs for the findings the audit reports. Each action refuses unless its finding holds, so it can only move the backlog toward a state `audit-issues` accepts. `normalize-status` maps a pre-3.0 legacy status (`deferred`) to its current equivalent (`backlog`); `--all-legacy` does every such item in one pass under one lock, which is what makes a several-hundred-item upgrade practical. |
| `append-issue` | `migrate-state` | Files a migration finding into the backlog rather than dropping it. Prefer the structured form — `--source-phase P --source-ref R [--source-note N]` — which DERIVES `--source` as `P (R)` so the two cannot disagree, and stores `source_phase`/`source_ref` for the auditor to read without parsing. Free-text `--source` is still accepted and stored verbatim for callers that have nothing structured to give. |
