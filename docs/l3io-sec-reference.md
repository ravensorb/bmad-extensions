...
`l3io-sec-redteam` is a memory-backed agent, not a stateless skill. It maintains a sanctum — a set of persistent identity and memory files at `{project-root}/_bmad/memory/l3io-sec-redteam/`. The sanctum holds the agent's persona, research cache, session logs, and accumulated knowledge about the project. Each session loads the sanctum and picks up where the previous one left off.
...
The agent maintains a cache of curated cloud and platform best practices at `{project-root}/_bmad/memory/l3io-sec-redteam/research-cache/`.
