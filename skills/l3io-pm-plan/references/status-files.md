...
| `audit-issues` | `--state-root [--format {text,json}]` — read-only integrity findings 1a–1j (key in both files, broken scheduling, missed done hook, unlinked promotion, stale `next`, bad status, archived undone story, double claim, duplicate/non-canonical key, fixed-but-reverted); a key with a resolved entry is checked only for 1a (resolved first), and a key duplicated in one file only for 1i; 1d/1h ignore a dead claim (a story under `archived/` that is not `done`); 1j fires only when the ref story's `resolves:` lists the key; exit 4 when any is found, and on a malformed issue file `--format json` prints `{"findings": [], "error": MSG}` |
