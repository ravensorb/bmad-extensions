...
Check if `l3io-sec-agent-redteam` is installed (look for `.claude/commands/l3io-sec-agent-redteam.md` or equivalent). If absent: announce "l3io-sec-agent-redteam not installed — red-team phase skipped", record the skip in `{status_file}`, and continue to Step 7.
