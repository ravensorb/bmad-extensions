...
| **many** `1f` legacy-status findings (a schema upgrade, not a sick backlog) | `uv run {pm_status} repair-issue --state-root {pm_state_root} --action normalize-status --all-legacy` — one confirmed pass, one lock, no `--key`. **Refuses (exit 2) when `deferred` and `backlog` coexist**, because that proves they were distinct states and the mapping would erase a decision; relay the refusal and leave the backlog alone. Offer this instead of the per-item command whenever more than a handful are legacy: a real upgrade produced 452, and per-item that is 452 subprocesses inside a per-item confirmation loop. The same gate applies to each item, so an unrecognised status is left alone rather than swept along |
