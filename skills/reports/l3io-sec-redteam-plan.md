...
**Single agent:** `l3io-sec-agent-redteam`. Interactive persona — asks clarifying questions about scope before running. Produces a structured findings report. Agent (not workflow) because scope disambiguation, follow-up questions, and nuanced analysis benefit from a conversational persona.
...
```
{project-root}/_bmad/memory/l3io-sec-agent-redteam/
...
**Relationships:** Called by `l3io-pm-sprint-execute` (scope=sprint) and `l3io-pm-epic-execute` (scope=epic). Can also be invoked standalone at solution scope.
...
| ---------- | ---- | ----------- | ----------- |
| WebSearch | Claude tool | l3io-sec-agent-redteam | Must be enabled in Claude Code permissions for live platform research |
...
1. **`l3io-sec-agent-redteam`** (Build an Agent) — Single skill module; build it, then go straight to Create Module (CM).
