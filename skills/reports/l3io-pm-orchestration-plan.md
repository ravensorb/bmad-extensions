...
9. Red-team review — `l3io-sec-agent-redteam` if installed (scope=sprint), else skip/fallback
...
2. Execute sprints — sequential by default; parallel when sprint groups are proven independent and status merges are serialized; each sprint spawns `l3io-pm-sprint-execute`; halt on BLOCKED/FAILED; confirm between sprints
...
4. *(parallel batch when outputs are isolated files)* **Clean release review** (epic/solution-scoped) + Adversarial review (`bmad-review-adversarial-general` across full epic) + Red-team review (`l3io-sec-agent-redteam` if installed, scope=epic) + UX review (conditional)
...
- **Epic → Sprint:** `l3io-pm-epic-execute` spawns `l3io-pm-sprint-execute` per sprint with: config path, status file, story keys, sprint number, epic root dir. Sprint writes closure outputs to disk; epic reads status line + targeted disk reads only.
- **Sprint → Security (optional):** At red-team review phase, sprint checks for `l3io-sec-agent-redteam`. If present, spawns with scope=sprint and sprint artifact path. If absent, skips with recorded note.
...
**Activation Modes:** Interactive (standalone invocation) and headless (called by l3io-pm-epic-execute).
...
- Optional: `l3io-sec-agent-redteam` (red-team review phase; skipped if absent)
...
**Relationships:** Called by `l3io-pm-epic-execute` as a subworkflow; calls `l3io-sec-agent-redteam` when installed.
...
**Tool Dependencies:**
- `l3io-pm-sprint-execute` (required — executes each sprint)
...
- Optional: `l3io-sec-agent-redteam`, `bmad-ux-review`
- Config: `{project-root}/_bmad/bmm/config.yaml`
...
**Relationships:** Calls `l3io-pm-sprint-execute` per sprint; calls `l3io-sec-agent-redteam` when installed.
...
| ---------- | ---- | ----------- | ----------- |
| `bmad-create-story` | BMad skill | l3io-pm-sprint-execute | Must be installed in target repo |
| `bmad-dev-story` | BMad skill | l3io-pm-sprint-execute | Must be installed in target repo |
| `bmad-code-review` | BMad skill | l3io-pm-sprint-execute | Must be installed in target repo |
| `bmad-qa-generate-e2e-tests` | BMad skill | l3io-pm-sprint-execute | Must be installed in target repo |
...
| `l3io-sec-agent-redteam` | l3io-sec module skill | Both workflows | Optional; red-team phase skipped if absent |
...
**l3io-sec integration:** When `l3io-sec-agent-redteam` is installed, both workflows automatically delegate red-team review to it. When absent, the phase is skipped with a recorded note. No configuration needed — detection is automatic.
...
- Red-team review steps: call l3io-sec-agent-redteam if security module is installed; otherwise use built-in fallback or skip
...
1. **`l3io-pm-sprint-execute`** (Build Workflow) — Build first because epic-execute depends on it. The sprint workflow is the core execution unit; getting it right makes epic straightforward.
2. **`l3io-pm-epic-execute`** (Build Workflow) — Build second. Shares the same orchestration patterns and subagent delegation model as sprint-execute. Pass this plan document as context so the builder understands the sprint workflow it calls.
