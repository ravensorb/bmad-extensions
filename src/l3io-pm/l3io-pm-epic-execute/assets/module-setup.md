...
Registers this standalone module into a project. Module identity (name, code, version) comes from `module.yaml` (sibling to this file). Collects user preferences and writes them to three files:
...
- **`{project-root}/_bmad/config.user.yaml`** — personal settings intended to be gitignored: `user_name`, `communication_language`, and any module variable marked `user_setting: true` in `module.yaml`. These values live exclusively here.
...
1. Read `module.yaml` for module metadata and variable definitions (the `code` field is the module identifier)
...
**Default priority** (highest wins): existing config values > `module.yaml` defaults.
...
Read each variable in `module.yaml` that has a `prompt` field. The module.yaml supports several question types:
...
Ask using the prompt with its default value. Apply `result` templates when storing values — e.g. a result template of `{project-root}/subdir/{value}` prepends the project root. Fields with `user_setting: true` go exclusively to `config.user.yaml`.
...
```bash
python3 scripts/merge-config.py --config-path "{project-root}/_bmad/config.yaml" --user-config-path "{project-root}/_bmad/config.user.yaml" --module-yaml module.yaml --answers {temp-file}
...
If `module.yaml` contains a `directories` array, also create each listed directory (resolving any `{field_name}` variables from the collected config values).
...
If `module.yaml` contains `post-install-notes`, display them (if conditional, show only the notes matching the user's selected config values).

Then display the `module_greeting` from `module.yaml` to the user.
