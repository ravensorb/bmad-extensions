...
Each orchestration phase records an epoch start timestamp (e.g. `{sprint_start_ts}`, `{epic_start_ts}`, `{story_start_ts}`). At close, sum the real `usage` fields from the session transcript JSONL entries for **this working directory** at/after that start. Discovery does **not** depend on the (undocumented) project-dir name encoding — it recurses all project dirs, honors `CLAUDE_CONFIG_DIR`, and scopes by each entry's `cwd` field:
...
```bash
find "$CFG/projects" -name '*.jsonl' -print0 2>/dev/null | xargs -0 cat 2>/dev/null \
  | jq -s --arg s "$START_ISO" --arg cwd "$PWD_NOW" '
      [ .[] | select((.timestamp // "") >= $s) | select((.cwd // "") == $cwd)
...
- **Discovery scopes by the entry `cwd` field** (matched to the current `pwd`) and recurses every project dir, so it is independent of the undocumented project-dir name encoding and captures subagent transcripts wherever Claude Code writes them.
- **Zero-entry guard:** if no usage rows match the window for this `cwd`, both helpers emit `N/A` — never `0` / `$0.00`. A false number is therefore impossible even if detection or discovery misfires (a non-Claude run, a relocated/renamed transcript, or a future Claude version that drops the `cwd` field) — the worst case is losing exact capture and recording `N/A`, never a fabricated value.
...
PowerShell does the same job natively (no `jq`/`find`/`xargs`). Use this on Windows, or anywhere PowerShell is the available shell. It reads the same transcripts, applies the same `cwd` scoping and the same zero-entry guard, and produces identical results.
...
$rows = Get-ChildItem (Join-Path $cfg 'projects') -Recurse -Filter *.jsonl -ErrorAction SilentlyContinue |
  Get-Content -ErrorAction SilentlyContinue |
  ForEach-Object { try { $_ | ConvertFrom-Json } catch {} } |
  Where-Object { $_.timestamp -ge $startIso -and $_.cwd -eq $pwdNow -and $_.message.usage }
